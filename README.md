...
- **(L8) is load-bearing at exactly one rung.** With the degree-two cap in hand,
  m = 12..20 die on (A)+(B)+(C)+the pair count+the cap alone, leaving nothing for
  (L8); only m = 21 needs it. Both peer audits went after the δ-budget inside
  (L8). That is now the narrowest target in the chain, not the widest.
